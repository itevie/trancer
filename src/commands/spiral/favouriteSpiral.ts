  ...
  handler: async (message) => {
    ...
    if (!message.reference)
      return message.reply(
        `Please reply to a message where the bot sent a spiral!`
      );
    ...
    // Add spiral
    await actions.spirals.favourites.addFor(message.author.id, spiral.id);
    return message.reply(
      `The spiral with ID **${spiral.id}** has been added to your favourites!`
    );
  },
};
