...
const command: HypnoCommand<{ date?: string }> = {
  ...
  usage: [
    [
      "$cmd dd-mm-yyyy",
      "See who gained the most currency since that date (if there is a snapshot for that date)",
    ],
  ],
  type: "economy",
  ...
  handler: async (message, { args, serverSettings }) => {
    ...
    if (args.date) {
      ...
      // Load it
      let oldData = JSON.parse(fs.readFileSync(path, "utf-8")) as {
        data: Economy[];
      };
      organised = organised.map((x) => [
        x[0],
        x[1] - (oldData.data.find((y) => y.user_id == x[0])?.balance ?? 0),
      ]);
