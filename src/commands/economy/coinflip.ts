  ...
  handler: async (message, { args, serverSettings }) => {
    ...
    if (requesterEco.balance < amount)
      return message.reply(
        `You do not have enough to coinflip **${amount}${config.economy.currency}**!`
      );
    if (requesteeEco.balance < amount)
      return message.reply(
        `The other person does not have enough to coinflip **${amount}${config.economy.currency}**!`
      );

    if (existingGames[message.author.id])
      return message.reply(
        `You have already requested a coinflip!\nIf you cannot find the cancel button, run \`${serverSettings.prefix}cancelcf\``
      );
    ...
    let msg = await message.reply({
      ...
      embeds: [
        createEmbed()
          .setTitle(
            `${args.user.username}, ${message.author.username} has requested a coinflip!`
          )
          .setDescription(
            `The coinflip is worth **${args.amount}${config.economy.currency}**!\n\n` +
    ...
    collector.on("collect", async (data) => {
      ...
      if (data.customId === "cancel") {
        ...
        let loser =
          message.author.id === winner.id ? args.user : message.author;

        // Check if the loser would go into debt
        if ((await getEconomyFor(loser.id)).balance < args.amount)
          return await m.edit(
            `Could not complete the coinflip, as the loser no longer has **${args.amount}${config.economy.currency}**`
          );

        setTimeout(async () => {
          await m.edit(
            `**${winner.username}** won the coinflip for **${args.amount}${config.economy.currency}**!`
          );
