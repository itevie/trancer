...
import { addMoneyFor, economyForUserExists, getEconomyFor, removeMoneyFor } from "../../util/actions/economy";
...
const command: HypnoCommand<{ user: User, amount: number }> = {
    ...
    handler: async (message, options) => {
        ...
        if (options.args.amount < 0 || options.args.amount % 1 !== 0)
            return message.reply(`Please provide a non-negative whole number to remove!`);

        // Add money
        await removeMoneyFor(options.args.user.id, options.args.amount);

        // Done
        return message.reply(`Successfully removed **${options.args.amount}${config.economy.currency} from ${options.args.user.username}**!`);
