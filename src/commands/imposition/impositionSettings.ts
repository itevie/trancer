...
const command: HypnoCommand = {
    ...
    handler: async (message, args) => {
        ...
        switch (args[0]) {
            ...
            case "chance":
                if (!args[1])
                    return message.reply(`Expected a second argument with the chance. Examples: 60, 46\nCurrent is: ${imposition.chance}%`);
            ...
            case "every":
                if (!args[1])
                    return message.reply(`Expected a second argument with the minutes. Examples: 10, 5\nCurrent is: ${imposition.every}`);
