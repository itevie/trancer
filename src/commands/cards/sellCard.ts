...
const command: HypnoCommand<{ card: Card; amount?: number; confirm?: string }> =
  {
    ...
    handler: async (message, { args }) => {
      ...
      if (price > 100 && !args.confirm)
        return message.reply(
          `Please provide the confirm option to sell **${amount} ${args.card.name}** for **${price}${config.economy.currency}**`
        );
      ...
      return message.reply(
        `You sold **${amount} ${args.card.name}**(s) for **${price}${config.economy.currency}**!`
      );
    },
  };
