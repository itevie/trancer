...
const command: HypnoCommand<{ user?: User }> = {
    ...
    handler: async (message, args) => {
        ...
        if (badges.length === 0) return message.reply(`${user.id === message.author.id ? "You" : "They"} have no badges :(`);
        return message.reply(`${user.id === message.author.id ? "Your" : "Their"} badges:\n\n${badges.join("\n")}`);
    }
};
