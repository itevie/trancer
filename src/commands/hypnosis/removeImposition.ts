...
const command: HypnoCommand = {
  ...
  usage: [["$cmd all", "Delete all your imposition"]],
  ...
  handler: async (message, { oldArgs: args }) => {
    ...
    if (args[0] === "all") {
      ...
      if (args[1] !== "confirm")
        return message.reply(
          `Dangerous action! This will delete ***ALL*** your imposition, type \`${serverSettings.prefix}removeimposition all confirm\` to do it.`
        );
    ...
    if (
      !(await database.get(
        `SELECT * FROM user_imposition WHERE user_id = (?) AND what = (?)`,
        message.author.id,
        what
      ))
    )
