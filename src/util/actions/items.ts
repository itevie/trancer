...
export async function getAquiredItem(
  id: number,
  userId: string
): Promise<Aquireditem> {
  let result = await database.get(
    `SELECT * FROM aquired_items WHERE item_id = ? AND user_id = ?;`,
    id,
    userId
  );
  if (!result)
    result = await database.get(
      `INSERT INTO aquired_items (item_id, user_id) VALUES (?, ?) RETURNING *`,
      id,
      userId
    );
  return result;
...
export async function getAquiredItemsFor(
  userId: string
): Promise<Aquireditem[]> {
...
export async function removeItemFor(userId: string, id: number): Promise<void> {
  await getAquiredItem(id, userId);
  await database.run(
    `UPDATE aquired_items SET amount = amount - 1 WHERE user_id = ? AND item_id = ?`,
    userId,
    id
  );
}
