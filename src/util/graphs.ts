...
import { getAllGuildsUserData } from "./actions/userData";
...
export async function generateGraph(creation: GraphCreationDetails): Promise<Buffer | string> {
    ...
    if (sourcePreSort.length > 0) {
        ...
        if (!(creation.sourceTableKey in sourcePreSort[0]))
            return graphError(`key ${creation.sourceTableKey} not in table ${creation.sourceTable}`);
    ...
    if (creation.users) {
        ...
        if (sortByPreSort.length > 0) {
            ...
            if (!(creation.sortByTableKey in sortByPreSort[0]))
                return graphError(`key ${creation.sortByTableKey} not in table ${creation.sourceTable}`);
            // Check if the key value is number
            if (typeof sortByPreSort[0][creation.sortByTableKey] !== "number")
                return graphError(`key ${creation.sortByTableKey}'s type is not of type number in table ${creation.sortByTable}`);
        }
    ...
    data = data.sort((a, b) => b[creation.sourceTableKey] - a[creation.sourceTableKey]);

    // Check if line is wanted (added_at is needed)
    if (creation.type === "line")
        if (data.length > 0 && !("added_at" in data[0]))
            return graphError(`Cannot create graph of type line, because the source table does not have an added_at key`);
